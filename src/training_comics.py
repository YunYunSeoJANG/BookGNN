...
if __name__ == '__main__':
    ...
    args = {
        ...
        'lr': get_config_value(config, 'lr', 2.136e-4),
        'loss_fn': "BPR",
        'epochs': get_config_value(config, 'epochs', 500),
        'num_layers': get_config_value(config, 'num_layers', 3),
        'conv_layer': get_config_value(config, 'conv_layer', 'LGC'),
        'neg_samp': get_config_value(config, 'neg_samp', 'random'),
        'gat_dropout': get_config_value(config, 'gat_dropout', 0.44),
    ...
    is_load_graph = not os.path.exists('assets/graph_kcore_comics_graphic.gpickle')
    if is_load_graph:
        ...
        tic = time.time()
        with open('assets/graph_kcore_comics_graphic.gpickle', 'rb') as f:
           G = pickle.load(f)
        toc = time.time()
